defmodule GatewayService.DownloadPodcastPlug do
  import Plug.Conn
  require Logger
  ...
  def download_podcast(conn) do
    ExternalService.call(:gateway_circuit_breaker, @retry_options, fn -> try_download_podcast(conn) end)
  end

  def try_download_podcast(conn) do
  ...
  def find_service(service_registry, params) do
    ...
    request_url = "#{service["address"]}:#{service["port"]}/download/#{params}"
  end
